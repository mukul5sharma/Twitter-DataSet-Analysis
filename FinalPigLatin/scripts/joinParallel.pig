...
-- Join the projected users dataset with the Networks dataset and the resulting table with the tweets dataset
joinUsersAndNetworks= JOIN projectedUsers BY (userId), networks BY (followerId) PARALLEL 20;
joinFollowersAndTweets= JOIN joinUsersAndNetworks BY (userId), tweets by (tuserId) PARALLEL 20;
...
-- Filter the projectedJoin table into time zones. 
...
STORE characterCentral INTO 's3://mrprojectsarika/outputParallel/central' USING PigStorage(',');
STORE characterEast INTO 's3://mrprojectsarika/outputParallel/east' USING PigStorage(',');
STORE characterMountain INTO 's3://mrprojectsarika/outputParallel/mountain' USING PigStorage(',');
STORE characterPacific INTO 's3://mrprojectsarika/outputParallel/pacific' USING PigStorage(',');
